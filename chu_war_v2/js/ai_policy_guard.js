...
(function (A) {
  ...
  function isEndgameHuntMode() {
    ...
    return phase() === "late" || enemy.total <= 8 || enemy.hidden <= 5;
  }
  ...
  function canReach(piece, fromRow, fromCol, toRow, toCol, board) {
    if (piece.type === "CAVALRY") {
      return (
        ...
        clearLine(fromRow, fromCol, toRow, toCol, board)
      );
    }
  ...
  function topKingPosition(board) {
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        if (piece && piece.owner === "top" && piece.type === "KING")
          return [row, col];
      }
    }
    return null;
  ...
  function bestHiddenCandidateNear(row, col) {
    ...
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        ...
        if (
          dist < bestDistance ||
          (dist === bestDistance && score > bestScore)
        ) {
          bestDistance = dist;
          bestScore = score;
        }
  ...
  function isImmediateBacktrack(fromRow, fromCol, move, piece) {
    ...
    return (
      sameSquare(previous.to, fromRow, fromCol) &&
      sameSquare(previous.from, move.r, move.c)
    );
  }
  ...
  function directAttackScore(piece, target, row, col, move) {
    ...
    if (target.revealed) {
      ...
      if (target.type === "KING" && (result === "A" || result === "X"))
        return 90000;
      if (result === "A")
        return 700 + pieceValue(target.type) - pieceValue(piece.type) * 0.15;
      if (result === "X")
        return (
          420 + pieceValue(target.type) * 0.55 - pieceValue(piece.type) * 0.55
        );
      if (result === "D" || result === "K")
        return (
          220 + pieceValue(target.type) * 0.35 - pieceValue(piece.type) * 0.35
        );
      return -90000;
    }
  ...
  function shouldKeepCavalryMove(
    fromRow,
    fromCol,
    move,
    piece,
    ...
    const moveDistance =
      move.dist || distance(fromRow, fromCol, move.r, move.c);
    ...
    if (target && target.owner === "bottom") {
      ...
      if (candidateScore < (nowPhase === "early" ? 210 : 170)) return false;
      if (
        nowPhase === "early" &&
        candidateScore < 285 &&
        hiddenNeighborCount(move.r, move.c) >= 1
      )
        return false;
      return true;
    }
    ...
    if (!target) {
      ...
      const usefulCandidatePressure =
        nearest.distance <= 2 &&
        nearest.score >= (nowPhase === "early" ? 230 : 190);
      const improvesCandidateDistance =
        nearest.score >= (nowPhase === "early" ? 220 : 180) &&
        nearest.distance + 1 < beforeNearest.distance;
      const usefulLinePressure = linePressure(move.r, move.c) >= 220;
      if (
  ...
  function shouldKeepHiddenAttack(
    fromRow,
    fromCol,
    move,
    piece,
    ...
    if (
      piece.type === "CAVALRY" &&
      (move.dist || distance(fromRow, fromCol, move.r, move.c)) >= 2
    )
      return true;
    ...
    if (
      piece.type === "G5" &&
      candidateScore < (nowPhase === "early" ? 330 : 285)
    )
      return false;
  ...
  function emergencyMoveScore(fromRow, fromCol, move, piece, currentDanger) {
    ...
    if (target && target.owner === "bottom") {
      const attackScore = directAttackScore(
        piece,
        target,
        move.r,
        move.c,
  ...
  function isEmergencyCounterplay(
    fromRow,
    fromCol,
    move,
    piece,
    ...
    return (
      emergencyMoveScore(fromRow, fromCol, move, piece, currentDanger) >= 650
    );
  }
  ...
  function emergencyFallbackMoves(fromRow, fromCol, moves, piece) {
    ...
    for (const move of moves) {
      ...
      if (
        ...
        continue;
      const score = emergencyMoveScore(
        fromRow,
        fromCol,
        move,
        piece,
  ...
  function shouldKeepMove(fromRow, fromCol, move, piece) {
    ...
    if (
      currentKingDanger >= 900 &&
      afterKingDanger >= currentKingDanger &&
      !isEmergencyCounterplay(fromRow, fromCol, move, piece, currentKingDanger)
    ) {
      return false;
    }
    ...
    if (piece.type === "BOMB" && !move.hit) {
      ...
      const beforeNearest = bestHiddenCandidateNear(fromRow, fromCol);
      const improvesCandidateDistance =
        nearest.score >= 240 && nearest.distance < beforeNearest.distance;
      if (
        !moveHelpsKing(fromRow, fromCol, move) &&
        !(nowPhase !== "early" && improvesCandidateDistance)
      )
        return false;
    }
    ...
    if (piece.type === "CAVALRY") {
      if (
        !shouldKeepCavalryMove(fromRow, fromCol, move, piece, target, nowPhase)
      )
        return false;
    }

    if (
      !shouldKeepHiddenAttack(fromRow, fromCol, move, piece, target, nowPhase)
    )
      return false;
  ...
  function chooseForcedCavalryStrike() {
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        const piece = A.S.board[row][col];
        if (!piece || piece.owner !== "top" || piece.type !== "CAVALRY")
          continue;
        ...
        for (const move of moves) {
          ...
          if (
            currentDanger >= 900 &&
            afterDanger >= currentDanger &&
            !isEmergencyCounterplay(row, col, move, piece, currentDanger)
          )
            continue;
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        for (const move of moves) {
          ...
          if (enemy.total <= 8 || enemy.hidden <= 5) score += 120;
  ...
  function runForcedHunt() {
    ...
    setTimeout(function () {
      ...
      const piece =
        A.S.board[hunt.fromRow] && A.S.board[hunt.fromRow][hunt.fromCol];
      const target =
        A.S.board[hunt.move.r] && A.S.board[hunt.move.r][hunt.move.c];
      if (
        piece &&
        piece.id === hunt.piece.id &&
        hunt.move &&
        (!hunt.move.hit || (target && target.owner === "bottom"))
      ) {
        A.S.logs.unshift(
          emergency
            ? "여름 AI 비상 대응: 왕 위협 중 최선 대응"
            : cavalryStrike
              ? "여름 AI 기병 직격: 장거리 공격 우선"
              : "여름 AI 긴급 추격: 후반 왕 후보 직접 공격",
        );
        A.applyMove(
          hunt.fromRow,
          hunt.fromCol,
  ...
  if (previousScheduleAiTurn) {
    A.scheduleAiTurn = function () {
      if (currentMode() === "ai-summer" && isAiTopTurn() && runForcedHunt())
        return;
