...
(function (A) {
  ...
  function isSummerTurn() {
    return mode() === 'ai-summer' && A.S && A.S.phase === 'playing' && A.S.turn === 'top';
  }
  ...
  function addCandidateMove(candidates, fromRow, fromCol, toRow, toCol, piece, kind, dist, targetScore) {
    ...
    const cost = { BOMB: 120, INFANTRY: 160, SPY: 190, CAVALRY: 310, G1: 360, G2: 500, G3: 620, G4: 760, G5: 900 }[piece.type] || 300;
    candidates.push({
      fromRow,
      fromCol,
      move: { r: toRow, c: toCol, kind: kind || 'normal', hit: true, dist: dist || distance(fromRow, fromCol, toRow, toCol) },
      piece,
      target,
    ...
    for (let fromRow = 0; fromRow < 8; fromRow++) {
      for (let fromCol = 0; fromCol < 8; fromCol++) {
        ...
        if (piece.type === 'CAVALRY') {
          if ((fromRow === row || fromCol === col) && clearLine(fromRow, fromCol, row, col)) {
            addCandidateMove(candidates, fromRow, fromCol, row, col, piece, 'normal', distance(fromRow, fromCol, row, col), targetScore);
          }
          continue;
        }

        if (distance(fromRow, fromCol, row, col) === 1) {
          addCandidateMove(candidates, fromRow, fromCol, row, col, piece, 'normal', 1, targetScore);
        }
        ...
        if (!piece.revealed && !piece.specialUsed) {
          ...
          if (piece.type === 'G1' && ((ar === 2 && ac === 1) || (ar === 1 && ac === 2))) {
            addCandidateMove(candidates, fromRow, fromCol, row, col, piece, 'special', ar + ac, targetScore);
          }

          if (piece.type === 'G2' && ((ar === 2 && ac === 0) || (ar === 0 && ac === 2))) {
            ...
            if (midPiece && midPiece.owner === 'top') {
              addCandidateMove(candidates, fromRow, fromCol, row, col, piece, 'special', 2, targetScore);
            }
          }
  ...
  function runForcedAttack(candidate) {
    ...
    setTimeout(function () {
      ...
      const piece = A.S.board[candidate.fromRow] && A.S.board[candidate.fromRow][candidate.fromCol];
      const target = A.S.board[candidate.move.r] && A.S.board[candidate.move.r][candidate.move.c];
      if (!piece || piece.owner !== 'top' || !target || target.owner !== 'bottom' || target.revealed) {
        pendingSummerTurn = false;
        return;
      }
      A.S.logs.unshift('여름 AI 긴급 왕 후보 공격');
      A.applyMove(candidate.fromRow, candidate.fromCol, candidate.move.r, candidate.move.c, candidate.move);
      releaseWhenTurnEnds();
    }, 520);
  }
