...
(function (A) {
  ...
  function finishMatch() {
    if (!currentMatch || currentMatch.phase === 'saved' || A.S.phase !== 'gameOver') return;
  ...
  function updateStatus() {
    ...
    if (latest && latest.result) {
      if (latest.result.winner) text += ' · 최근: ' + A.label(latest.result.winner) + ' 승리';
  ...
  function copyLatestLog() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () {
        if (A.alert) A.alert('경기 로그', '최근 경기 로그를 클립보드에 복사했습니다.');
      });
    } else {
  ...
  function clearLogs() {
    writeLogs([]);
    updateStatus();
    if (A.alert) A.alert('경기 로그', '브라우저에 저장된 경기 로그를 초기화했습니다.');
  }
  ...
  function bindButtons() {
    ...
    if (passButton && passButton.onclick && !passButton.dataset.matchLogWrapped) {
  ...
  if (originalApplyMove) {
    A.applyMove = function (fromRow, fromCol, toRow, toCol, move) {
      ...
      const before = {
        ...
        dist: move && move.dist || Math.abs(fromRow - toRow) + Math.abs(fromCol - toCol)
      };
  ...
  if (originalAlert) {
    A.alert = function (title, body, callback) {
      const wrappedCallback = callback ? function () {
